"""Procedural OpenCV Anime & Illustration Renderer."""
from typing import Optional
import cv2
import numpy as np

from src.art.types import StylePreset
...
class OpenCVIllustrationRenderer:
    ...
    def render(
        ...
        # 4. Cinematic Lighting (Warm Studio Key Light + Cool Ambient Shadows)
        ...
        # 5. Extract Dynamic Ink Lines & Multiply
        line_art = self.extract_line_art(rgb, edge_map=control_map.edge_map)
        line_f = line_art.astype(np.float32) / 255.0
        composite = cel * line_f

        # 6. Global Contrast & Saturation Boost
